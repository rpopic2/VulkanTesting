...
private:
    ...
    void createInstance()
    {
        ...
        //calling vkEnumerateInstanceExtensionProperties twice IS a valid code since we need to retrive extensionCount first.
        
        std::cout << extensionProps.size() << " extensions avaialable :\n";
        for (const auto &extensionsAvailable : extensionProps)
        {
            std::cout << '\t' << extensionsAvailable.extensionName << '\n';
        }
    ...
    //[swap chains] there are three properties for swap chain :
    // 1. surface format, 2. present mode, 3. extent
    ...
    void createGraphicsPipeline() {
        ...
        vertexInputInfo.pVertexBindingDescriptions = nullptr;//opt
        vertexInputInfo.vertexAttributeDescriptionCount = 0;
        vertexInputInfo.pVertexAttributeDescriptions = nullptr;//opt
        // * input asm
        ...
        pipelineLayoutInfo.setLayoutCount = 0; //opt
        pipelineLayoutInfo.pSetLayouts = nullptr; //opt
        pipelineLayoutInfo.pushConstantRangeCount = 0; //opt
        pipelineLayoutInfo.pPushConstantRanges = nullptr; //opt
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }
        
        // * finally the graphic pipeline
        ...
        pipelineInfo.pDepthStencilState = nullptr;//opt
        ...
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; //opt
        pipelineInfo.basePipelineIndex = -1; //opt
